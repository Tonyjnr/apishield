  ...
  function processItems(items, basePath = '') {
    for (const item of items) {
      if (item.item) {
        ...
        // Check auth
        const hasAuth = !!(item.request.auth || 
          (item.request.header && item.request.header.some(h => 
            h.key === 'Authorization' || h.key === 'X-API-Key'
          )));
...
module.exports = { parsePostman, normalizePostman };
